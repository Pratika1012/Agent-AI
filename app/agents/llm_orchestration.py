...
#         """Load API keys and model selection logic."""
...
#         elif any(word in q_lower for word in ['code', 'script', 'program']):
#             return "qwen-2.5-coder-32b"
#         else:
#             return "mixtral-8x7b-32768"

#     def call_model(self, model_name: str, query: str) -> str:
#         """Calls the Groq or Anthropic LLMs dynamically using LangChain."""
#         start_time = time.time()
#         api_key = self.api_keys["groq"]  # Default to Groq API Key
...
from Utils.logger import setup_logger
from Utils.token_counter import count_tokens  
from agents.vector_db import VectorDB  # ✅ Import Pinecone DB for memory storage
...
class LLMOrchestrator:
    def __init__(self, CONFIG_PATH="config/Configration.json"):
        ...
        # ✅ Initialize Pinecone VectorDB for memory storage
        self.memory = VectorDB(pinecone_api_key=self.api_keys["pinecone"])
    ...
    def generate_response(self, query: str) -> str:
        ...
        past_context = self.memory.retrieve_similar(query, k=1)

        # If past context exists, use it to modify the query
        if past_context:
            query = f"Previous context: {past_context[0]}\nNew Query: {query}"
