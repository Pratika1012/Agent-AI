...
class ResearchAgent:
    ...
    def choose_prompt_style(self, query: str) -> str:
        """Dynamically selects the best prompting style based on query complexity."""

        # ✅ Attempt Groq API Classification
        classification_prompt = f"""
        Classify the following query into one of three categories based on complexity:
        ...
        **Respond with ONLY one of these categories:**
        ...
        raw_response = self.call_groq_api(classification_prompt)
        self.logger.info(f"[DEBUG] Groq API Classification Response: {raw_response}")

        style_response = extract_category(raw_response)

        # ✅ If Groq repeatedly returns Chain-of-Thought → Use Dynamic Heuristic Classification
        if style_response == "chain_of_thought":
            style_response = self.dynamic_complexity_analysis(query)
            self.logger.info(f"[Fallback] Switching to dynamic complexity analysis: {style_response}")
    ...
    def dynamic_complexity_analysis(self, query: str) -> str:
        ...
        # ✅ **Tree-of-Thought** (Complex, Multi-Part Queries)
        if word_count > 12 or any(keyword in query_lower for keyword in ["compare", "contrast", "differences", "break down", "steps"]):
            return "tree_of_thought"
